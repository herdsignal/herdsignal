...
- **Dashboard**: SPY 군중 상태, 종목 검색, 선택형 자산 패널과 보유 현황을 한 화면에서 확인
...
- React는 SPY 시장 관찰, 종목 검색, 선택형 자산 패널을 합친 대시보드와
  관찰 목록·종목 상세 화면을 구성합니다.
...
대시보드와 연구 화면에서는 최신 관찰 기준일과 모델 상태를 확인할 수 있습니다.
...
사용자 판단 피드백은 기존 판단 기록에 당시 HERD와 1·3·6개월 경로로 남습니다. 이 기록은
회고용이며 사용자의 판단을 모델 정답으로 간주하거나 자동 학습·임계값 조정·모델 승격에
사용하지 않는 계약을 `data/config/personal_feedback_policy_v1.json`으로 고정했습니다.
웹 화면의 전체 데이터 수동 갱신은 인증이 꺼진 로컬 모드 또는
`HERDSIGNAL_OWNER_EMAIL`과 일치하는 Google 계정에서만 실행됩니다.
...
- SEC 가이던스는 범용 파서 개발을 중단하고 원문 검수된 atomic fact만
  사용했습니다. 수정쌍 coverage는 확보했지만 경영진 가이던스 하향 가설이
  독립 OOS 채택 기준을 통과하지 못해 행동 근거로 사용하지 않습니다.
