...
- **개인 행동 기준**: 상태 점수와 분리해 목표 비중·투자 방식에 따른 검토 범위 설정
...
- **현재 상태**: `NO_ADOPTABLE_CANDIDATE`

가격·추세·상대강도·위험·기업 상태 가설을 독립 OOS로 검증했지만 현재까지
운영 가능한 익절·추가매수 방향 증거는 0개입니다. Blind holdout은 열지
않았고 행동 비율도 활성화하지 않았습니다. 현재는 SEC 8-K 가이던스를
접수 시각 기준으로 복원해 새로운 기업 기대 변화 증거가 될 수 있는지
검증하는 단계입니다.
...
- SEC 가이던스 구조 파서 V6는 V5 오답 10건을 교정했지만, 독립 표본이 52건·12개 기업에 그쳐 80건·20개 기업 및 Wilson 95% 하한 90% 게이트를 아직 통과하지 않았습니다.
