...
| 상태 | 의미 | 현재 대표 묶음 |
| --- | --- | --- |
| `ACTIVE` | 다음 연구 판정에 직접 사용 | SEC 가이던스 구조 파서 V2와 새 80건 검수 |
...
현재 활성 연구 사슬은 공개 데이터 계약 → 입력 manifest V2 → SEC 8-K corpus
→ 구조 파서 V2 → 독립 80건 원문 검수다. 검수 전에는 수정쌍, 방향 가설,
가격 OOS, HERD 가중치로 진행하지 않는다.
