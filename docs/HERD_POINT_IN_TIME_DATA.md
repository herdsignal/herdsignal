...
- 독립 서술문 사건 추출을 포함한 공식 구성 해결: 294/397건
- 공식 표로 공개본 날짜 교정: 56건
- 공식 서술문으로 공개본 날짜 교정: 12건
- SEC 동일 CIK와 적용일이 검증된 ticker 변경: 11건
- 구성 변경 후보에서 동일성 변경으로 재분류한 행: 22건
- 현재 미해결 후보 행: 81건
- 공식 S&P 보도자료 corpus: 137건
...
같은지는 원문을 확인해야 한다. 미해결 81건은 자동 추정하지 않는다.
...
아래 진단도 기존 157건 원장을 사용한 이전 결과다. 최신 305건 조정
결과의 공식 일별 재생으로 오해하지 않는다.

최신 v3 진단은 공식 구성 294건과 ticker 변경 11건을 재생했다. 차단
사건 81건, 오류 6건, 최종 구성 494개로 아직 진단 전용이다.
