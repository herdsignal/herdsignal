...
최신 통합 원장은 공식 구성 사건 259건, SEC 기업행동 보강 사건 42건,
ticker 변경 16건, 기업 구성 연속성 5건, 차단 사건 36건으로 구성된다.
공개본의 표기 진동과 즉시 반전 20행은 구성 순효과가 0임을 확인한 뒤
원장 밖으로 격리했다.

EVHC·DXC는 공식 S&P 편입 발표와 SEC 합병 원문을 함께 확인했고,
NLOK·PEAK는 동일 CIK의 공식 ticker 변경 원문을 확인했다. PEAK의 선행
HCP가 공개 기준 구성에서 누락된 문제는 2016-10-24 S&P 발표의 “기존
S&P 500 구성”과 “분할 후 잔류” 문구로 확인했다. 다만 기준일 이후의
근거이므로 HCP 추가는 공식 기준 수정이 아닌 진단용 연속성 역추론이다.

DISCA·DISCK가 WBD 단일 클래스로 합쳐지는 사건은 공식 S&P 잔류 문구와
SEC 8-K의 전환·거래일을 모두 확인했다. 재생기는 여러 이전 ticker를
하나의 새 ticker로 합치는 동일성 사건을 원자적으로 처리한다.

2016-07-18 원본 480종목에 HCP를 진단 보정한 481종목으로 322개 검증 사건을
재생하면 구조 오류는 0건, 최종 구성은 493종목이다. 차단 36건은 다음
작업 대기열로 분리했다.

- P0 공식 구성 행동·적용 세션 의미 검토: 5건
- P2 근접 날짜 사건의 회사 동일성 선행 조사: 19건
- P2 누락 S&P 공식 문서 탐색: 12건

모든 대기 사건은 `promotion_allowed=false`다. 오류 0건은 원장의 완전성을
뜻하지 않으며, 공개 기준 구성과 차단 사건이 남아 있어
`replay_ready=false`, `survivorship_safe=false`다.

### 단일 파이프라인 최신 결과

후보 수를 목표로 한 예외 처리를 제거하고 다음 불변식을 코드로 고정했다.

- 티커 별칭은 CIK별 유효 구간이 겹치지 않아야 한다.
- 새 티커 구간은 predecessor와 거래 시작일을 가져야 한다.
- 기업행동은 합병일·거래 시작일·지수 적용일을 별도 저장한다.
- 같은 날 편입 후 rename은 명시적 sequence로 실행한다.
...
기업 승계 사건으로 정규화했다. SEC 원문만 있는 세 승계는
`DIAGNOSTIC_CORPORATE_CONTINUITY`로 분리되어 공식 사건으로 승격되지 않는다.

합병 전 BHI와 합병 후 BKR의 S&P 500 구성 근거를 SEC 승계 원문과
교차검증해 BHI→BHGE 연속성을 공식 승격했다. 증분 S&P 근거 보관기는
서로 다른 CSV 열을 합집합으로 보존하며, 공식 문서 13건을 해시 고정한다.

최신 진단 재생은 구조 오류 0건, 최종 구성 500종목, 차단 사건 4건이다.
남은 핵심은 PX 기준 구성 원문 해시 고정과 PX→LIN 승계, 그리고 세 진단
...
5. P0 5건의 공식 구성 행동·적용 세션 의미 검증
6. P2 19건의 회사 동일성·CIK·ticker 선행 조사
7. P2 12건의 누락 공식 문서 재탐색
8. 교체 쌍·구성 수·거래일 불변식 검사
9. 통합 원장 재생성 및 일별 구성 재생
