...
- v6: Progressive Action Layer로 신호 생애주기, 신뢰도, 포트폴리오 리스크, 알림 조건 반영
- v7: 섹터별 파라미터 분리와 피크아웃/바닥 확인 백테스트
- v8: ML 기반 가중치 최적화

## 3. Now

- MVP 집중: Dashboard, Watchlist, Search, HERD Lab
- HERD_v6 Progressive Action Layer 실사용 검증
- 실제 사용 중 HERD 신호 신뢰도, 판단 기록, 알림 조건 품질 점검

## 4. Next

- 1~2주 로컬 실사용 후 Dashboard/Watchlist/Search 마찰 제거
- StockDetail 판단 기록을 장기 매매 일지로 더 확장
- 일간/주간 확인 루틴과 실제 알림 채널 설계
- HERD_v7 후보: Rush/Flee 내부 강도와 피크아웃/바닥 확인 백테스트
- 종목별 HERD 백테스트 카드
...
- 저마찰 포트폴리오 import 흐름
- 실제 알림 시스템: 강한 Flee/Rush, 목표비중 이탈, 장기 신호 지속만 대상으로 제한
...
- HERD 알고리즘, v1~v4
- 전체 데이터 파이프라인
- HERD_v5 Balanced Action Layer, HERD_v6 Progressive Action Layer
- Dashboard: Signal Command Center, SPY Herd Flow, Action Queue, 현금 포함 포트폴리오 요약, 총자산 히스토리, 리스크 체크, 알림 조건
- StockDetail: 결론, 근거, 신뢰도, HERD 히스토리, 재무 가드, 판단 기록과 결과 추적
- Watchlist: 매수 대기열, 준비도 기반 Action Queue 리스트
