...
│   ├── finnhub_collector.py    Finnhub 수집기 (심볼 검색, EPS 서프라이즈 보정)
...
- 운영 계산에는 아직 반영하지 않으며, 수익률 보존율과 MDD 개선이 기준을 통과할 때 backend Action Layer로 승격한다.
- 채택 기준은 수익률 보존율 70% 이상, MDD 개선 5%p 이상, 연평균 거래 수 4~10회 수준이다.
...
| `FINNHUB_API_KEY` | — | Finnhub 키 (심볼 검색·EPS 서프라이즈 보정·뉴스·애널리스트) |
...
- `backtest_action_layer.py`는 Action Layer 후보 검증용이며, 운영 HERD 점수와 API 응답에는 아직 영향을 주지 않음
