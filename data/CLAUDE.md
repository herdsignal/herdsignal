...
| 200주 MA 위치 | 20% | `ma200_weekly` | 활성, 단 DB 저장 미구현 |

> **주의**: `ma200_weekly`는 `calculator.py`에서 계산되지만 `saver.py`의 `_upsert_herd_indicators`에 저장 코드가 없음.
> 따라서 `herd_indicators` DB 테이블과 API 응답(`HerdScoreResponse`)에 포함되지 않음.
...
- `herd_indicators` — 5개 지표 분해값 (ma200_weekly 제외)
