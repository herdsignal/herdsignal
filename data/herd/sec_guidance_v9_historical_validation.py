"""2010~2016 독립 코퍼스를 V4→V9로 재생해 원문 검수 표본을 잠근다."""
...
from herd.sec_guidance_structure_parser_v9 import audit_v8_review, transform_candidate as transform_v9
...
def build(protocol: dict) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    ...
    candidates = _transform(v8, transform_v9)
    if not candidates.empty:
        candidates["review_priority"] = candidates.apply(lambda row: hashlib.sha256(
            f'{row["source_sha256"]}:{row["source_structure"]}:{row["range_offset"]}:{row["metric"]}:V9-HIST'.encode()
        ).hexdigest(), axis=1)
    ...
    report = {
        "report_version": "herd-sec-guidance-v9-historical-validation-v1",
        ...
        "next_decision": "COMPLETE_LOCKED_V9_SOURCE_REVIEW" if ready else "V9_HISTORICAL_SAMPLE_COVERAGE_BLOCKED",
