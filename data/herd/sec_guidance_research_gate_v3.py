"""독립 V3 원문 정확도에 따라 SEC 가이던스 연구 3~8을 fail-closed한다."""
...
V4_REQUIREMENTS = [
    "PRESERVE_TABLE_ROW_COLUMN_AND_HEADER_HIERARCHY",
    "BIND_PERIOD_METRIC_BASIS_UNIT_AND_ROLE_PER_RANGE",
    "DISTINGUISH_REPORTING_COMPARISON_AND_GUIDANCE_PERIODS",
    "DISTINGUISH_ACTUAL_PRIOR_CURRENT_AND_RESPECTIVE_SINGLE_VALUES",
    "PRESERVE_SIGN_AND_CASH_FLOW_DIRECTION",
    "BIND_REPORTED_ADJUSTED_NORMALIZED_AND_EXCLUSION_BASIS",
    "REJECT_HISTORICAL_GUIDANCE_RECAPS_WITHOUT_CURRENT_UPDATE",
]
...
def build_gate_state(protocol: dict, review: dict, labels: pd.DataFrame) -> dict:
    ...
    for index, name in enumerate(protocol["steps"], start=1):
        if index == 1:
            status = "COMPLETE"
            reason = (
                f"{review['reviewed_rows']} unseen rows across "
                f"{review['distinct_tickers']} issuers adjudicated"
            )
        elif index == 2:
    ...
    return {
        "report_version": "herd-sec-guidance-research-gate-v3",
        "decision": "PARSER_V4_REQUIRED" if not passed else "BUILD_SOURCE_QUALIFIED_REVISION_PAIRS",
        "steps": steps,
        ...
        "next_parser_requirements": V4_REQUIREMENTS,
    }
