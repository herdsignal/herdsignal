"""V1~V9 미검수 accession에서 마지막 증분 파서 V10 표본을 잠근다."""
...
def build(protocol: dict) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    ...
    if not candidates.empty:
        ...
        candidates["review_priority"] = candidates.apply(
            lambda row: hashlib.sha256(
                ...
                f'{row["range_offset"]}:{row["metric"]}:V10'.encode()
            ).hexdigest(),
            axis=1,
        )
    ...
    if not review.empty:
        review.insert(0, "review_id", [f"SG10-{number:04d}" for number in range(1, len(review) + 1)])
    ...
    report = {
        "report_version": "herd-sec-guidance-v10-independent-validation-v1",
        "input_candidates": len(source),
        "v10_candidates": len(candidates),
        "v10_candidate_tickers": int(candidates["ticker"].nunique()) if not candidates.empty else 0,
        ...
        "next_decision": "COMPLETE_LOCKED_V10_SOURCE_REVIEW" if ready else "V10_SAMPLE_COVERAGE_BLOCKED",
        "price_outcomes_observed": False,
        "operational_action_ratio": 0.0,
    }
    report.update(audit_v9_review(protocol["development_reviews"][-1]))
