"""표적 SEC cover 원장 V3로 FINRA ticker-date의 PIT CIK coverage를 재감사한다."""
...
SEC_PROTOCOL = Path(__file__).with_name("sec_cover_page_targeted_ledger_v1.json")
SEC_REPORT = ROOT / "data/reports/sec_cover_page_targeted_ledger_v1.json"
SEC_LEDGER = ROOT / "data/reports/sec_time_valid_ticker_cik_intervals_v3.csv"
...
class FinraCoverageV3Error(RuntimeError):
    pass
...
def load_sec_intervals(
    ...
    if report["protocol_sha256"] != _sha256(protocol_path):
        raise FinraCoverageV3Error("SEC targeted ledger protocol lineage mismatch")
    if report["ledger_sha256"] != _sha256(ledger_path):
        raise FinraCoverageV3Error("SEC targeted interval ledger hash mismatch")
    ...
    if len(intervals) != report["verified_interval_count"]:
        raise FinraCoverageV3Error("SEC verified interval count mismatch")
    return intervals, {
        ...
        "targeted_document_count": report["targeted_document_count"],
...
def audit_v3(
    ...
    overrides = protocol["finra_v3_policy"]["cohort_symbol_overrides"]
    ...
    result = audit(
        ...
        report_version="FINRA_SHORT_INTEREST_COVERAGE_AUDIT_V3",
        next_priority=(
            "KEEP_FINRA_IN_PROSPECTIVE_SHADOW_AND_DO_NOT_TUNE_"
            "A_DIRECTION_HYPOTHESIS_ON_THE_COVERAGE_SAMPLE"
        ),
        cohort_symbol_overrides=overrides,
    )
    required = set(protocol["finra_v3_policy"]["required_cohorts"])
    required_reports = [
    ...
    result["identifier_ledger"]["bny_research_ticker_resolved_to"] = "BK"
    result["identifier_ledger"]["finra_bny_assigned_to_bny_mellon"] = False
...
def main() -> None:
    ...
    result = audit_v3(args.report, args.detail)
    print(json.dumps({
