"""새 기업 코퍼스를 기존 결합기와 V7 교정기로 재생해 독립 검수 표본을 잠근다."""
...
def build(protocol: dict) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    ...
    v4_protocol = {"development_reviews": protocol["development_reviews"], "review_gate": protocol["review_gate"]}
    v4, _, v4_report = build_v4([corpus], load_aliases(Path(protocol["alias_registry"])), v4_protocol)
    v5 = pd.DataFrame([item for _, row in v4.iterrows() if (item := transform_v5(row)) is not None])
    ...
    v6 = pd.DataFrame([item for _, row in v5.iterrows() if (item := transform_v6(row, locator)) is not None])
    candidates = pd.DataFrame([item for _, row in v6.iterrows() if (item := transform_v7(row)) is not None])
    if not candidates.empty:
        candidates["review_priority"] = candidates.apply(lambda row: hashlib.sha256(
            f'{row["source_sha256"]}:{row["source_structure"]}:{row["range_offset"]}:{row["metric"]}:V7'.encode()
        ).hexdigest(), axis=1)
    ...
    if not review.empty:
        review.insert(0, "review_id", [f"SG7-{index:04d}" for index in range(1, len(review) + 1)])
    ...
    return candidates, review, {
        "report_version": "herd-sec-guidance-v7-independent-validation-v1",
        "input_documents": int(pd.read_csv(corpus / "index.csv")["source_sha256"].nunique()),
        "v4_candidates": v4_report["v4_candidates"], "v7_candidates": len(candidates),
        "v7_candidate_tickers": int(candidates["ticker"].nunique()) if not candidates.empty else 0,
        "development_accessions_excluded": len(excluded),
        "review_rows": len(review), "review_tickers": int(review["ticker"].nunique()) if not review.empty else 0,
        "review_sample_gate_ready": ready, "review_gate_passed": False,
        "ready_for_direction_preregistration": False,
        "next_decision": "COMPLETE_LOCKED_V7_SOURCE_REVIEW" if ready else "V7_INDEPENDENT_SAMPLE_COVERAGE_BLOCKED",
        "price_outcomes_observed": False, "operational_action_ratio": 0.0,
    }
