...
def build_current_state(root: Path = ROOT) -> dict[str, Any]:
    ...
    sec_8k_identity_promotion_v2 = _load(root / SEC_8K_IDENTITY_PROMOTION_V2.relative_to(ROOT))
    sec_8k_hard_adverse_corpus_v2 = _load(root / SEC_8K_HARD_ADVERSE_CORPUS_V2.relative_to(ROOT))
    ...
    if sec_8k_structural_extraction["parser_changed_after_lock"]:
        contradictions.append("SEC 8-K parser changed during independent extraction")
    if sec_8k_structural_extraction["human_labels_created"] != 0:
        contradictions.append("SEC 8-K independent extraction creates human labels")
    if sec_8k_structural_extraction["identity_promotion_allowed"]:
    ...
    if (
        sec_8k_structural_independent_review["identity_promotion_allowed"]
        and sec_8k_structural_independent_review["status"]
        != "INDEPENDENT_SOURCE_PRECISION_GATE_PASSED"
    ):
        contradictions.append("SEC 8-K independent review promotes identity before gate pass")
    ...
    if sec_8k_identity_promotion_v2["open_ended_intervals_inferred"] != 0:
        contradictions.append("SEC 8-K promotion infers open-ended ticker intervals")
    if sec_8k_hard_adverse_corpus_v2["operational_action_ratio"] != 0.0:
        contradictions.append("SEC 8-K corpus V2 has action authority")
    if sec_8k_remaining_identity_audit_v2["unmapped_events"] != sec_8k_hard_adverse_corpus_v2["unmapped_events"]:
        contradictions.append("SEC 8-K remaining identity audit changes the unmapped population")
    if sec_8k_remaining_identity_audit_v2["same_cik_anchor_implies_interval"]:
    ...
    return {
        ...
        "sec_8k_structural_extraction": {
            ...
            "independent_extraction_status": sec_8k_structural_extraction["status"],
            "independent_candidate_rows": sec_8k_structural_extraction[
                "candidate_rows"
            ],
            "independent_no_candidate_rows": sec_8k_structural_extraction[
                "no_candidate_rows"
            ],
            ...
            "same_cik_anchor_review_events": sec_8k_remaining_identity_audit_v2[
                "legacy_with_same_cik_anchor"
            ],
            "historical_master_required_events": sec_8k_remaining_identity_audit_v2[
                "legacy_without_same_cik_anchor"
            ],
