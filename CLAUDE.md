...
- Dashboard: S&P 500 Herd Flow 배너(Overview 애니메이션 + Timeline HERD 히스토리), 총자산/현금 포함 포트폴리오 평가 요약, 편집 모드 현금 입력, 총자산 히스토리 차트, KRW/USD 통화 토글, 핵심 리밸런싱 체크, HERD 신호와 목표비중 차이를 함께 반영한 보유 종목 액션 카드, 편집 모드, 평단가·수량 수정 모달, localStorage 캐시, 빠른 새로고침 피드백
