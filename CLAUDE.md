...
- StockDetail: HERD v4 점수·단계·신호, HERD_v5 Action Layer 행동 비율, 현재 신호 근거 데이터 보드, HERD 판단 기록(가격·수량·총액·수익률·메모)과 기록 요약, 현재 신호 기준 신뢰도, HERD Index 히스토리 차트, Fundamental Guard, 지표 분해·보정 승수
...
- HERD Lab: 현재 HERD 모델 버전(`HERD_v5`) 검증 데이터 보드, 목표 대비 PASS/WATCH 판정, 종목별 백테스트 verdict, 5단계 행동 매트릭스. 표시 데이터는 `frontend/src/data/herdModelReport.js`에서 관리한다.
